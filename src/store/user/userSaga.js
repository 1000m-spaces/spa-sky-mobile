...
function* getListUserAffiliate() {
  const currentUser = yield asyncStorage.getUser();
  try {
    const result = yield call(
      UserController.getListUserAffiliates,
      currentUser?.custid,
    );
    if (result.success === true) {
      yield put({
        type: NEOCAFE.GET_LIST_USER_AFFILIATE_SUCCESS,
        payload: result.list,
      });
    } else {
      yield put({
        type: NEOCAFE.GET_LIST_USER_AFFILIATE_ERROR,
        payload: result.message,
      });
    }
  } catch (error) {
...
export default function* watcherSaga() {
  ...
  yield takeLatest(
    NEOCAFE.GET_LIST_USER_AFFILIATE_REQUEST,
